...
export default function SubjectBookPage({ subject, subjectName }) {
  ...
  const fetchPsirPreview = async () => {
    console.log(`[SubjectBookPage] Fetching preview for subject '${subject}'...`);
    setIsLoading(true);
    ...
    try {
      ...
      console.log(`[SubjectBookPage] Preview loaded. ${hierarchy.length} paper(s): ${hierarchy.map((p) => p.paper).join(', ') || '(none)'}.`, data);
  ...
  const saveLayoutForPaper = async (paper) => {
    ...
    try {
      await fetch(`${API_BASE_URL}/api/subjects/${subject}/layout`, {
        method: "POST",
      ...
      console.error("[SubjectBookPage] Failed to save layout:", err);
    }
  };
  ...
  const handleSelectionChange = (qId, urlStr) => {
    setSelections((prev) => {
      ...
      if (current.includes(urlStr))
        return { ...prev, [qId]: current.filter((u) => u !== urlStr) };
      if (current.length >= 3) {
  ...
  const isAllActiveSelected = () => {
    ...
    return (
      activeIds.length > 0 && activeIds.every((id) => includedQuestions.has(id))
    );
  };
  ...
  const toggleTopicCollapse = (topicTitle) => {
    setCollapsedTopics((prev) => ({
      ...prev,
      [topicTitle]: prev[topicTitle] !== false ? false : true,
    }));
  };
  ...
  const moveQuestionToPosition = (tIndex, fromIndex, toIndex) => {
    setPsirData((prev) => {
      ...
      if (toIndex < 0 || toIndex >= questions.length || toIndex === fromIndex)
        return prev;
  ...
  const moveTopic = (tIndex, direction) => {
    ...
    setPsirData((prev) => {
      ...
      [topics[tIndex], topics[targetIndex]] = [
        topics[targetIndex],
        topics[tIndex],
      ];
  ...
  const generateAndPreviewPdf = async () => {
    ...
    try {
      const orderedIncludedIds = activePaperNode.topics.flatMap((t) =>
        t.questions
          .filter((q) => includedQuestions.has(q._id))
          .map((q) => q._id),
      );
      if (orderedIncludedIds.length === 0)
        throw new Error(
          "Please select at least one question to include in the book.",
        );
      ...
      console.log(`[SubjectBookPage] Generating book for subject '${subject}', paper '${activePaper}', ${orderedIncludedIds.length} question(s) included.`);
      ...
      if (!response.ok) {
        const errData = await response.json();
        throw new Error(
          errData.error || "Failed to start PDF book generation.",
        );
      }
      const data = await response.json();
      console.log(`[SubjectBookPage] Generation queued. Job ID: ${data.jobId}`);
      pollJobStatus(data.jobId);
    } catch (err) {
      console.error('[SubjectBookPage] Generate failed:', err);
      alert(err.message);
      setIsGenerating(false);
    }
  };
  ...
  const pollJobStatus = (jobId) => {
    console.log(`[SubjectBookPage] Polling job '${jobId}' status every 4s...`);
    const intervalId = setInterval(async () => {
      try {
        const statusResponse = await fetch(
          `${API_BASE_URL}/api/subjects/${subject}/status/${jobId}`,
        );
        if (!statusResponse.ok)
          throw new Error("Failed to retrieve compilation progress.");
        const jobData = await statusResponse.json();
        console.log(`[SubjectBookPage] Job '${jobId}' status: '${jobData.status}'.`);
        setGenerationStatus(jobData.status);
        if (jobData.status === "completed") {
          clearInterval(intervalId);
          console.log(`[SubjectBookPage] Job '${jobId}' completed.`);
          setPdfBlobUrl(`${API_BASE_URL}/api/subjects/${subject}/download/${jobId}`);
          setIsGenerating(false);
          setShowPreviewModal(true);
        } else if (jobData.status === "failed") {
          clearInterval(intervalId);
          console.error(`[SubjectBookPage] Job '${jobId}' failed:`, jobData.error);
          setIsGenerating(false);
          alert(`PDF Generation failed: ${jobData.error || "Unknown error"}`);
        }
      } catch (err) {
        console.error("[SubjectBookPage] Polling error:", err);
        clearInterval(intervalId);
        setIsGenerating(false);
        alert(err.message);
      }
  ...
  const downloadFinalPdf = async () => {
    ...
    console.log(`[SubjectBookPage] Downloading final PDF from: ${pdfBlobUrl}`);
    ...
    try {
      ...
      console.log(`[SubjectBookPage] Download fetched: ${blob.size} byte(s).`);
      const localUrl = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = localUrl;
      link.download = `Formal_${subject}_${activePaper.replace(/[^a-z0-9]/gi, "_")}.pdf`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(localUrl);
    } catch (err) {
      console.error("[SubjectBookPage] Download failed:", err);
  ...
  // Cloudinary serves raw-resource files with Content-Disposition: attachment by
  // default, which forces a download instead of letting the browser preview it inline
  // (and cascades into multiple auto-downloads when several toppers render at once).
  // Proxying through our backend lets us override the headers to inline.
  const getPreviewUrl = (url) => `${API_BASE_URL}/api/subjects/${subject}/preview-file?url=${encodeURIComponent(getCleanUrl(url))}`;

  const cleanupStorage = async () => {
    const confirmed = window.confirm(
      `This will permanently delete every compiled ${subjectName} book file stored on the server (including any not yet downloaded) to free up database space. Job history and selections are kept. Continue?`,
    );
    if (!confirmed) return;

    console.log(`[SubjectBookPage] Cleaning up storage for subject '${subject}'...`);
    setIsCleaningStorage(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/subjects/${subject}/cleanup-storage`, {
        method: "POST",
      });
      if (!response.ok) {
        const errData = await response.json();
      ...
      const data = await response.json();
      console.log('[SubjectBookPage] Cleanup result:', data);
      alert(
      ...
      console.error('[SubjectBookPage] Cleanup failed:', err);
      alert(err.message);
    } finally {
    ...
    <div className="min-h-screen bg-[#0f172a] text-gray-100 p-6 md:p-12 font-sans relative flex flex-col items-center">
      ...
      <div className="max-w-6xl w-full text-center mb-10 relative">
        <button
          ...
          title={`Delete all compiled ${subjectName} book files from server storage to free up space`}
        ...
        <h1 className="text-3xl font-bold tracking-tight text-white mb-2">
          {subjectName} Topic-Wise Book Builder
        </h1>
        <p className="text-gray-400 max-w-xl mx-auto text-sm">
          Generate separate high-quality compiled answer books based on the {subjectName}{" "}
          questions dataset. Filter questions, pick matching topper papers,
          reorder sheets, and compile them instantly.
        </p>
      </div>
              ...
              (paperName) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] mt-0.5 font-bold ${isActive ? "text-indigo-200" : "text-gray-500"}`}
                    >
                      {paperNode ? paperNode.section : "Loading..."} ({totalQ}{" "}
                      Qs)
                    </span>
                  </button>
                );
            ...
            <div className="flex flex-col gap-6 animate-in fade-in duration-200">
              ...
              <div className="bg-gray-800/40 border border-gray-700/60 p-6 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 backdrop-blur-sm shadow-sm">
                <div>
                  ...
                  <h2 className="text-xl font-bold text-white mt-0.5">
                    {activePaperNode.section}
                  </h2>
                  <p className="text-xs text-gray-400 mt-1">
                    Manage {totalActiveQuestions} questions grouped by topics.
                    Type a position number next to a question to move it there
                    instantly. Use ↑↓ to move whole topics up/down.
                  </p>
                </div>
              ...
              <div className="space-y-4">
                {activePaperNode.topics.map((topNode, tIndex) => {
                  ...
                  const topicIncludedCount = topNode.questions.filter((q) =>
                    includedQuestions.has(q._id),
                  ).length;
                  const isEditingThisTopic = editingTopicIndex === tIndex;

                  return (
                    <div
                      key={`${tIndex}-${topNode.title}`}
                      className="bg-gray-800/20 border border-gray-800/80 rounded-2xl overflow-hidden shadow-inner"
                    >
                      {/* Topic Header */}
                      <div
                        onClick={() =>
                          !isEditingThisTopic &&
                          toggleTopicCollapse(topNode.title)
                        }
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          >
                            ...
                            <button
                              onClick={() => moveTopic(tIndex, 1)}
                              disabled={
                                tIndex === activePaperNode.topics.length - 1
                              }
                              className="p-1 rounded bg-gray-800 hover:bg-gray-700 disabled:opacity-20 disabled:cursor-not-allowed text-gray-400 hover:text-white transition-colors cursor-pointer"
                          ...
                          {isEditingThisTopic ? (
                            ...
                            >
                              <input
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === "Enter")
                                    saveTopicName(tIndex, topNode.title);
                                  if (e.key === "Escape")
                                    setEditingTopicIndex(null);
                                }}
                        ...
                        <span className="text-xs font-semibold px-3 py-1 rounded-full text-gray-400 shrink-0 ml-3">
                          {topicIncludedCount} / {topNode.questions.length}{" "}
                          selected
                        </span>
                      </div>
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-start gap-3 mb-3">
                                  ...
                                  {/* Include checkbox */}
                                  <button
                                    onClick={() => toggleIncludeQuestion(q._id)}
                                                    ...
                                                    {isEditingThisTopper ? (
                                                      ...
                                                      >
                                                        <input
                                                          ...
                                                          value={
                                                            editingTopperValues.topper_name ||
                                                            ""
                                                          }
                                                          onChange={(e) =>
                                                        ...
                                                        <div className="grid grid-cols-3 gap-1">
                                                          <input
                                                            placeholder="Year"
                                                            value={
                                                              editingTopperValues.topper_year ||
                                                              ""
                                                            }
                                                            onChange={(e) =>
                                                              setEditingTopperValues(
                                                          ...
                                                          <input
                                                            placeholder="Rank"
                                                            value={
                                                              editingTopperValues.topper_rank ||
                                                              ""
                                                            }
                                                            onChange={(e) =>
                                                              setEditingTopperValues(
                                                          ...
                                                          <input
                                                            placeholder="Marks"
                                                            value={
                                                              editingTopperValues.topper_marks ||
                                                              ""
                                                            }
                                                            onChange={(e) =>
                                                              setEditingTopperValues(
              ...
              <div className="bg-gray-850 p-4 border border-gray-800 rounded-xl text-center text-xs text-gray-400">
                ...
                out of{" "}
                <strong className="text-white">{totalActiveQuestions}</strong>{" "}
                questions in{" "}
            ...
            <div className="p-4 border-b border-gray-800 bg-gray-900/90 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold text-white mb-0.5">
                  {activePaper} PDF Preview
                </h2>
                <p className="text-xs text-gray-400">
                  Review layout & page organization before exporting
                </p>
              </div>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl flex flex-col items-center text-center animate-in zoom-in-95">
            ...
            <p className="text-gray-400 text-xs mb-6 max-w-xs leading-relaxed">
              We have offloaded PDF compilation to GitHub Actions. This prevents
              server timeouts and ensures high performance.
            </p>
            <div className="w-full bg-gray-950 border border-gray-800 rounded-xl p-4 flex flex-col gap-3.5 mb-6 text-left">
              <div className="flex items-center gap-3">
                ...
                />
                <span
                  className={`text-xs font-bold ${generationStatus === "pending" ? "text-white" : "text-gray-400"}`}
                >
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <span
                  className={`text-xs font-bold ${generationStatus === "completed" ? "text-white" : "text-gray-600"}`}
                >
