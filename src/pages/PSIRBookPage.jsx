...
export default function PSIRBookPage() {
    ...
    const intervalId = setInterval(async () => {
      try {
        const statusResponse = await fetch(
          `${API_BASE_URL}/api/psir/status/${jobId}`,
        );
  ...
  const getPreviewUrl = (url) => `${API_BASE_URL}/api/psir/preview-file?url=${encodeURIComponent(getCleanUrl(url))}`;
  ...
  const cleanupStorage = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/psir/cleanup-storage`, {
        method: "POST",
      });
      if (!response.ok) {
        const errData = await response.json();
    ...
    <div className="min-h-screen bg-[#0f172a] text-gray-100 p-6 md:p-12 font-sans relative flex flex-col items-center">
      ...
      <div className="max-w-6xl w-full text-center mb-10 relative">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-white mb-2">
          PSIR Topic-Wise Book Builder
        </h1>
        <p className="text-gray-400 max-w-xl mx-auto text-sm">
          Generate separate high-quality compiled answer books based on the PSIR
      ...
      {isLoading ? (
        ...
        <div className="max-w-6xl w-full flex flex-col gap-6">
          ...
          <div className="w-full flex bg-gray-900 border border-gray-800 p-1.5 rounded-2xl gap-2 shadow-inner">
            {["Paper 1A", "Paper 1B", "Paper 2A", "Paper 2B"].map(
              (paperName) => {
